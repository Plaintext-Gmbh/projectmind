/// Pure timeline + range logic shared by the bean-graph *cinematics* player
/// (`bean-graph-live`, V4.3 / #66 concept 2 — "press play over a commit
/// range") and the code-city *time-lapse* (`code-city`, V5 — "watch the city
/// grow"). The stateful mounts, the player toolbars and the actual per-step
/// fetch/paint live in `app/src/components/BeanGraphLive.svelte` and
/// `app/src/components/CodeCity.svelte`; this module holds the part that can
/// be unit-tested without a DOM: turning the per-module `commit_activity`
/// payload into one global, evenly sampled commit timeline, and mapping a
/// player step onto the `{from, to}` ref pair the existing
/// `listChangesSince(from, to)` API diffs.
///
/// Extracted verbatim from `beanGraphCinematics.ts` for V5 (that path stays
/// as a re-export shim, so the cinematics call sites and its test are
/// untouched — the unchanged test is the proof nothing moved behaviourally).
/// The type names keep their `Cinematics*` spelling: renaming them would
/// ripple through BeanGraphLive for zero behavioural gain.
///

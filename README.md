...
- **C4 model (editable)** — the round-trip sibling of C4 container ([#142](https://github.com/Plaintext-Gmbh/projectmind/issues/142)): `scaffold_c4_model` writes `docs/architecture.dsl` (a Structurizr-DSL subset) once, you hand-edit it in Git, and this kind parses it back to Mermaid. ProjectMind never overwrites the file, so the architecture is yours to own — JVM-free, no Structurizr CLI. When the code grows, the view's **Update model** button (or the `merge_c4_model` tool) folds the new modules, classes and relations in **additively** — your own edits, hand-added elements and comments are preserved byte-for-byte. **Export draw.io** (or the `export_c4_drawio` tool) turns the model into `docs/architecture.drawio` — draw.io's own C4 shapes, one page for the container view plus one per container with components — and opens it in the embedded diagrams.net editor, where **Edit** lets you polish and save straight back into the repo. Re-exporting is a layout-preserving merge: every cell carries the model id in its metadata, so your arrangement survives and only new elements are added.
...
| `export_c4_drawio` | Export the C4 model to a **hand-editable draw.io diagram** ([#142](https://github.com/Plaintext-Gmbh/projectmind/issues/142)): writes `docs/architecture.drawio` from the editable model (`docs/architecture.dsl` when present, else the generated model) using draw.io's built-in C4 shapes — page 1 the container view, one page per container with components. Every cell carries `pmId` / `pmKind` metadata, so re-running is a **layout-preserving merge**: existing cells (and the architect's manual layout) are kept byte-for-byte, only missing elements are added below the drawing, nothing is deleted. Legacy compressed pages are decompressed on read; output is plain XML. The desktop app and browser viewer open the file in an embedded diagrams.net editor with save-back. Returns `{ path, created, added, kept, pages }`. |
...
- **Phase 2 (in progress):** Annotation round-trip, ~~draw.io embed~~ (`.drawio` files render **and edit** via the embedded diagrams.net editor with save-back; the C4 model exports to draw.io), ~~Confluence / Jira bridge~~ (external references — Confluence, Jira, issue trackers, doc URLs — surface in the class viewer), dynamic plugin loading from `./plugins/`.
- **Phase 3:** Plugin marketplace, more languages, JSF / PrimeFaces preview, ~~C4 diagram generator~~ (shipped: generated C4 container / component views, editable model, draw.io export).
